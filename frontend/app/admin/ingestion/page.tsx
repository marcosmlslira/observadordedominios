...
export default function IngestionPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [runsData, allRunsData, summaryData, policyData, coverageData, bulkJobsData, countsData] =
        await Promise.all([
  ...
  // Build domain count lookup for coverage table
                      ...
                      coverage.map((item) => (
                        <TableRow key={item.tld}>
                          ...
                          <TableCell className="text-right tabular-nums text-xs">
                            {domainCountMap.has(item.tld)
                              ? domainCountMap.get(item.tld)!.toLocaleString()
                              : "—"}
                          </TableCell>
                          <TableCell>
